// 聊天消息面板 — 从 Electron 版迁移;指令执行改为 motorController.sendCommand
...
import { consumePendingToolCall } from './ChatApp'
...
export const ChatPane: React.FC = () => {
  ...
  const confirm = async () => {
    ...
    lock.setExecuting()
    try {
      const r = await sendCommand(pt.name, pt.args)
      useSessionStore.getState().applyLlmEvent({ type: 'text', content: `>> ${r}` })
    } catch (e) {
      useSessionStore.getState().applyLlmEvent({ type: 'error', message: `执行失败: ${e}` })
      lock.unlock()
    }
    setPt(null)
  }
