...
```
ai_motor_control/       → GitHub main (x86, libasound2)
ai_motor_control_arm/   → GitHub arm64 (ARM, libasound2)
```

ARM 差异：`package.json` arch + `libasound2` + 串口设备名文档，源码 `rsync -a` 同步后 `sed -i 's/libasound2/libasound2/g'`。
...
```bash
sudo apt install -y libnss3 libnspr4 libasound2  # x86
