...
export interface CommandLock { sessionId: string; toolCallId: string; status: LockStatus; lockedAt: number }
...
export interface TelemetryData { type: 'telemetry'; rpm: number; current: number; seriesRpm: number[]; seriesIa: number[]; channels?: Record<string, number[]> }
...
export type BackendEvent = BackendStarted|SerialStatus|TelemetryData|BackendError|BackendInterrupted|BackendExecuted|BackendPong
