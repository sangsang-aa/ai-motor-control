...
export const ChatPane: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (pc) { setPt(pc); setExpired(false); tr.current = setTimeout(() => { setExpired(true); setPt(null) }, TIMEOUT) }
  ...
  const confirm = async () => {
    ...
    try {
      const r = await window.api.sendCommand(pt.name, pt.args)
      useSessionStore.getState().applyLlmEvent({ type: 'text', content: `>> ${r}` })
    } catch (e) { useSessionStore.getState().applyLlmEvent({ type: 'error', message: `执行失败: ${e}` }) }
    setPt(null)
  }
  const ignore = () => { if (tr.current) clearTimeout(tr.current); setPt(null) }
      ...
      <div ref={sr} className="chat-scroll">
        {msgs.length === 0 && !inflight && (
          <div className="welcome">
            ...
            <div className="t">AI 电驱控制系统</div>
            <div className="h">输入自然语言指令控制电机，例如"把转速提高到 3000 转"</div>
          </div>
        )}
        {msgs.map(m => (
          <div key={m.id} className={`msg-row ${rc(m.role)}`}>
            <div className={`msg-bbl ${rc(m.role)}`}>{m.content}{m.streaming && <span className="stream-dot" />}</div>
          </div>
        ))}
